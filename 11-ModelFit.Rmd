...
Likewise, a single target model’s $χ^2$ statistic can be thought of as a $Δχ^2$ statistic using equations \@ref(eq:11-16) and \@ref(eq:11-17), where Model B is replaced with the saturated Model S. In our illustrative example of child anxiety the overall $χ^2$ is then obtained by:
$$
...
Recall from equation 11.03 that a model’s χ2 statistic is the LRT from equation 9.06, treating the target model as Model A and the saturated model as Model B.  The $Δχ^2$ test is also a log-likelihood ratio, so equation 11.15 could equivalently be calculated using equation 11.03.  The reason the two equations yield the same result is that each model’s $χ2$ statistic is calculated using the same saturated model’s log-likelihood ($ℓS_$), so they cancel out. Labelling the saturated model as Model S instead of B in equation 11.03, the LRT for two nested hypothetical Models A (in \color{green}{green}) and B (in \color{blue}{blue}) could be calculated using each of their LRT statistics:
