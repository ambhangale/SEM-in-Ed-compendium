...
This chapter will show how to fit a multigroup SEM to raw data as well as to summary statistics.  For the latter, we require group-specific summary statistics, which Chapter 1 discussed how to obtain:
...
<!--chapter:end:02-Intro2lavaan.Rmd-->
...
We will use example data provided on the [\textcolor{blue}{M\textit{plus} website}](http://www.statmodel.com) for a technical report (Muthén, 2011, p. 24).  The example data can be downloaded onto your computer from the report's [\textcolor{blue}{supplementary materials}](http://www.statmodel.com/examples/penn.shtml#extendSEM), or you can provide the URL directly to `read.table(file=)`, as shown below.  Note that the data are in summary format (i.e., a table with frequencies for each combination of categories for the 3 variables).  The `read.table()` output is therefore transformed into a standard `data.frame` with 1 row per subject.
...
$$\text{odds} = \frac{\pi}{1 - \pi} = e^{\text{Logit}(\pi)} \text{ , where } \; \; e = `r exp(1)`$$

$$\pi = \frac{\text{odds}}{1 + \text{odds}} = \frac{e^\text{Logit}}{1 + e^\text{Logit}}$$
...
The probit model above can therefore be reframed in terms of this LRV, $y^*$.

$$y^* = \mathbf{XB} + \varepsilon ,$$
where $\varepsilon \sim \mathcal{N}(0, \sigma)$ and the observed binary response $y$ is linked to the LRV by a *threshold model*:
...
In either case, the threshold is the same distance from the LRV's mean, as depicted in the figure below.
...
The plot below visualizes the "classification rules" above.

```{r plot poly, echo=FALSE}
...
Consider the mediation model depicted below.  The dashed line is the direct effect of treatment on smoking behavior, given the intent to smoke.
...
\node[manifest] (x) at (0,0) {$y_1$\\$\text{T}_\text{x}$};
\node[manifest] (m) at (2,2) {$y_2$\\Intent};
\node[manifest] (y) at (4,0) {$y_3$\\Smoked};
...
<!--chapter:end:12-IntroCategorical.Rmd-->
...
\node[manifest] (y2) at (2,2) {$y_2$};
\node[manifest] (y3) at (4,2) {$y_3$};
\node[manifest] (y4) at (6,2) {$y_4$};
...
\node[manifest] (y5) at ( 9,2) {$y_1$};
\node[manifest] (y6) at (11,2) {$y_2$};
\node[manifest] (y7) at (13,2) {$y_3$};
\node[manifest] (y8) at (15,2) {$y_4$};
...
\node[manifest] (y2) at ( 4,0) {$y_2$};
\node[manifest] (y3) at ( 8,0) {$y_3$};
\node[manifest] (y4) at (12,0) {$y_4$};
...
\begin{align}
\text{Level 1: } y_{i,t} &= \beta_{i,0} + \beta_{i,1} \text{Time}_{i,t} + \varepsilon_{i,t} = \textcolor{blue}{\Lambda \mathbf{\eta}_i + \varepsilon_i} \label{eq:L1} \\
  ...
  \beta_{i,1} &= \textcolor{blue}{(\eta_1=)} \gamma_{1,0} + \gamma_{1,1} \text{Age}_i + \gamma_{1,2} \text{Peer-Influence}_i + u_{1,i} \label{eq:L2slope+pred} \\
  ...
  &+ u_{0,i} + u_{1,i} \text{Time}_{i,t} + \varepsilon_{i,t} \ \ \textcolor{gray}{\text{(joint residual)}} \label{eq:L2res}
\end{align}
...
<!--chapter:end:26-LatentCurves.Rmd-->
