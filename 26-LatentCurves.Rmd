# Latent Growth Curve Models {#ch26}
...
\begin{align} 
\mathrm{y}_{i,t} &= \beta_{i,0} + \varepsilon_{i,t} (\#eq:26-1) \\
        &= (\gamma_{0} + u_i) + \varepsilon_{i,t} (\#eq:26-2)
\end{align}

indicating that each Subject $i$ has their own intercept, so the random intercept $\beta_{i,0}$ is itself a variable.  As Eq. \@ref(eq:26-2) shows, the average intercept can be depicted as the sum of 2 components:
...
```{r label="fig26-1", engine='tikz', fig.ext='svg', echo = FALSE, fig.cap="Random intercept model for alcohol-use data from Duncan and Duncan (1996)." }
...
The following `lavaan` model syntax specifies the model from Figure \@ref(fig:fig26-1).
...
```{r, engine='tikz', fig.ext='svg', label="fig26-2", echo = FALSE, fig.cap="Linear growth model for alcohol-use data from Duncan and Duncan (1996)." }
\begin{tikzpicture}[
latent/.style={draw, circle, thick, minimum size=2cm, inner sep=0pt, align=center, execute at begin node=\setlength{\baselineskip}{1.2em}},
error/.style={draw, circle, thick, minimum size=1cm, inner sep=0pt, align=center, execute at begin node=\setlength{\baselineskip}{1.2em}},
manifest/.style={draw, rectangle, thick, minimum size=1cm, align=center, execute at begin node=\setlength{\baselineskip}{1.2em}},
...
```{r, engine='tikz',fig.ext='svg', label="fig26-3", echo = FALSE, fig.cap="Latent basis models for alcohol-use data from Duncan and Duncan (1996)."}
...
The script below specifies the model depicted in the left panel of Figure \@ref(fig:fig26-3).
```{r basis12, echo=-1}
...
Another example of a latent basis curve fixes the first and *last* loadings to 0 and 1, respectively, with similar implications for interpretation.  The script below specifies the model depicted in the right panel of Figure \@ref(fig:fig26-3).
```{r basis14, echo=-1}
...
Finally, because there is a growth factor for each occasion, the entire covariance structure can be reproduced in the latent space (factor (co)variances).  Thus, we must fix indicators' residual variances to zero, as depicted in Figure \@ref(fig:fig26-4).

```{r, engine='tikz',fig.ext='svg', label="fig26-4", echo = FALSE, fig.cap="Saturated growth model for alcohol-use data from Duncan and Duncan (1996)." }
...
The script below specifies the model in Figure \@ref(fig:fig26-4), fits the model, and prints the estimated latent means for interpretation.
```{r sat mod, echo=-1}
#TODO: add figure
mod.sat <- ' ## loadings
...
Y_i &= \beta_0 + \beta_1 X_{i1} + \beta_2 X_{i2} + \beta_3 X_{i1} X_{i2} + \varepsilon_i \\
    ...
    &= \vartheta_0 + \vartheta_1 X_{i2} + \varepsilon_i (\#eq:26-3)
\end{align}


By grouping the terms in the second row of Eq. \@ref(eq:26-3), we can reinterpret it as a simple regression of $Y$ on $X_2$ (the third row of Eq. \@ref(eq:26-3)).  But the intercept and slope are themselves variables because they depend on (are functions of) $X_1$.

- Simple intercepts: $\vartheta_0 = \beta_0 + \beta_1 X_{i1}$
- Simple slopes: $\vartheta_1 = \beta_2 + \beta_3 X_{i1}$

Recall the moderation is symmetric, so the terms in Eq. \@ref(eq:26-3) could also be grouped such that the effect of $X_1$ depends on $X_2$.  Moderation can also be interpreted in both directions in a LGCM, but it is not as obvious how because time is not an explicit variable in the model, as other predictors are.
...
Level-2 predictors vary between subjects, so they can explain between-subject differences in trajectories, which are represented by growth factors.  Although Duncan and Duncan (1996) had several such covariates, our example data include the following two subject-level predictors: `age` at the beginning of the study and `peer` encouragement to consume illicit substances.  Consider the MLM for growth, which extends Eq. \@ref(eq:26-3) by adding time as a predictor.  Below, we intersperse some equivalent SEM symbols \color{blue}{in blue} as reminders of how the MLM and SEM frameworks map onto each other (Bauer, 2003; Curran, 2003; Singer & Willett, 2003, ch. 8).

```{=tex}
\begin{equation}
\text{Level 1: } y_{i,t} = \beta_{i,0} + \beta_{i,1} \text{Time}_{i,t} + \varepsilon_{i,t} = \color{blue}{\Lambda \mathbf{\eta}_i + \varepsilon_i}
(\#eq:26-4)
\end{equation}
```

\begin{align}
\text{Level 2: } \beta_{i,0} &= \color{blue}{(\eta_0=)} \gamma_{0,0} + \gamma_{0,1} \text{Age}_i + \gamma_{0,2} \text{Peer-Influence}_i + u_{0,i} \\
\beta_{i,1} &= \color{blue}{(\eta_1=)} \gamma_{1,0} + \gamma_{1,1} \text{Age}_i + \gamma_{1,2} \text{Peer-Influence}_i + u_{1,i} (\#eq:26-5)
\end{align}
...
```{r, engine='tikz',fig.ext='svg', label="fig26-5", echo = FALSE, fig.align = "center", fig.cap="Linear growth model for alcohol-use data from Duncan and Duncan (1996) with subject-level predictors of growth factors." }
\begin{tikzpicture}[
latent/.style={draw, circle, thick, minimum size=2cm, inner sep=0pt, align=center, execute at begin node=\setlength{\baselineskip}{1.2em}},
error/.style={draw, circle, thick, minimum size=1cm, inner sep=0pt, align=center, execute at begin node=\setlength{\baselineskip}{1.2em}},
manifest/.style={draw, rectangle, thick, minimum size=1cm, align=center, execute at begin node=\setlength{\baselineskip}{1.2em}},
...
\end{tikzpicture}
```
The LGCM script below specifies a measurement model for the growth factors (Eq. \@ref(eq:26-)) and a structural model that includes both predictors of initial status (Eq. \@ref(26-4)) and rate of change (Eq. \@ref(eq:L2slope+pred)).  We will consider the combined model in Eqs. \@ref(eq:26-5)--\ref{eq:L2res} further when we interpret the structural model.
...
There are 3 predictors with simple effects in Eq. \@ref(eq-26-5): `age`, `peer`, and Time.  But their effects are not represented in the same way in a LGCM.  First, we focus on the effects of time-invariant predictors.
...
Notice that Eq. \@ref(eq:26-6) only includes a fixed (not random) effect of cigarette use. The possibility of including a random effect for both Time and cigarette use is one of the ways in which MLM is more flexible than SEM.  Notice also that Eq. \@ref(eq:26-6) includes a Time $\times$ cigarette-use interaction effect ($\beta_3$).  This is how moderation is represented in a regression model, but moderation of Time by a Level-1 variable in LGCM is quite different, involving more than one parameter. 
...
```{r, engine='tikz',fig.ext='svg', label="fig26-6", echo = FALSE, fig.align = "center", fig.cap="Linear growth model for alcohol-use data from Duncan and Duncan (1996), controlling for cigarette-use data." }
...
The script below specifies the curve-of-factors model depicted in Figure \@ref(fig:fig26-7).
```{r curve of factors}
