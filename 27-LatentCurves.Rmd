...
```{r label="fig27-1", engine='tikz', fig.ext='svg', echo = FALSE, fig.cap="Random intercept model for alcohol-use data from Duncan and Duncan (1996)." }
...
```{r, engine='tikz', fig.ext='svg', label="fig27-2", echo = FALSE, fig.cap="Linear growth model for alcohol-use data from Duncan and Duncan (1996)." }
\begin{tikzpicture}[
latent/.style={draw, circle, thick, minimum size=2cm, inner sep=0pt, align=center, execute at begin node=\setlength{\baselineskip}{1.2em}},
error/.style={draw, circle, thick, minimum size=1cm, inner sep=0pt, align=center, execute at begin node=\setlength{\baselineskip}{1.2em}},
manifest/.style={draw, rectangle, thick, minimum size=1cm, align=center, execute at begin node=\setlength{\baselineskip}{1.2em}},
...
```{r, engine='tikz',fig.ext='svg', label="fig27-3", echo = FALSE, fig.cap="Latent basis models for alcohol-use data from Duncan and Duncan (1996)."}
...
Finally, because there is a growth factor for each occasion, the entire covariance structure can be reproduced in the latent space (factor (co)variances).  Thus, we must fix indicators' residual variances to zero, as depicted in Figure \@ref(fig:fig27-4).

```{r, engine='tikz',fig.ext='svg', label="fig27-4", echo = FALSE, fig.cap="Saturated growth model for alcohol-use data from Duncan and Duncan (1996)." }
...
\beta_{i,1} &= \color{blue}{(\eta_1=)} \gamma_{1,0} + \gamma_{1,1} \text{Age}_i + \gamma_{1,2} \text{Peer-Influence}_i + u_{1,i} (\#eq:27-5)
\end{align}
...
```{r, engine='tikz',fig.ext='svg', label="fig27-5", echo = FALSE, fig.align = "center", fig.cap="Linear growth model for alcohol-use data from Duncan and Duncan (1996) with subject-level predictors of growth factors." }
\begin{tikzpicture}[
latent/.style={draw, circle, thick, minimum size=2cm, inner sep=0pt, align=center, execute at begin node=\setlength{\baselineskip}{1.2em}},
error/.style={draw, circle, thick, minimum size=1cm, inner sep=0pt, align=center, execute at begin node=\setlength{\baselineskip}{1.2em}},
manifest/.style={draw, rectangle, thick, minimum size=1cm, align=center, execute at begin node=\setlength{\baselineskip}{1.2em}},
...
\end{tikzpicture}
```
The LGCM script below specifies a measurement model for the growth factors (Eq. \@ref(eq:27-)) and a structural model that includes both predictors of initial status (Eq. \@ref(27-4)) and rate of change (Eq. \@ref(eq:L2slope+pred)).  We will consider the combined model in Eqs. \@ref(eq:27-5)--\ref{eq:L2res} further when we interpret the structural model.
...
There are 3 predictors with simple effects in Eq. \@ref(eq-27-5): `age`, `peer`, and Time.  But their effects are not represented in the same way in a LGCM.  First, we focus on the effects of time-invariant predictors.
...
Notice that Eq. \@ref(eq:27-6) only includes a fixed (not random) effect of cigarette use. The possibility of including a random effect for both Time and cigarette use is one of the ways in which MLM is more flexible than SEM.  Notice also that Eq. \@ref(eq:27-6) includes a Time $\times$ cigarette-use interaction effect ($\beta_3$).  This is how moderation is represented in a regression model, but moderation of Time by a Level-1 variable in LGCM is quite different, involving more than one parameter. 
...
```{r, engine='tikz',fig.ext='svg', label="fig27-6", echo = FALSE, fig.align = "center", fig.cap="Linear growth model for alcohol-use data from Duncan and Duncan (1996), controlling for cigarette-use data." }
...
```{r, engine='tikz',fig.ext='svg', label="fig27-7", echo = FALSE, fig.align = "center", fig.cap="Curve-of-factors model for substance-use data from Duncan and Duncan (1996)." }
