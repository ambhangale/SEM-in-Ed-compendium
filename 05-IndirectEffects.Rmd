...
A single indirect effect is calculated by multiplying specific elements from matrix $\mathbf{B}$. For example, `B[3,1]` selects the element in row 3, column 1 from the $\mathbf{B}$ matrix, and this element is multiplied with element `B[4,3]`. This is the indirect effect of Parental anxiety on Child anxiety via Overcontrol (see Figure 1). We gave this effect the name `I1`, because you may want to calculate more indirect effects, which could then be named `I2`, `I3`, `I4` etc.
