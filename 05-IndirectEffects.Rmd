...
In the model of Figure 5.3, there are two indirect effects from PA on CA. One path goes from PA to PO and then from PO to CA, and is equal to $β_{31} * β_{43}$. The other indirect effect goes from PA to PP, from PP to PO, and from PO to CA, and is calculated by $β_{21} * β_{32} * β_{43}$. Because this effect is build up of three direct effects, this is called a third order effect. The sum of all specific indirect effects is the total indirect effect. The total effect of PA on CA equals the sum of all indirect effects plus the direct effect ($β_{41}$). This model also contains an indirect effect of PA on PO, which equals $β_{21} * β_{32}$.
