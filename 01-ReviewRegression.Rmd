...
The SEM software that we will use is an open-source R package called [`lavaan`](https://lavaan.ugent.be/) [(Rosseel, 2012)](http://dx.doi.org/10.18637/jss.v048.i02). To install the `lavaan` package you can type the following command in the R console:
...
Covariances are explained in more detail in a later section, but it is sufficient here to understand that the covariance between variables $x$ and $\mathrm{y}$ (i.e., $\sigma_{xy}$) is their unstandardized correlation coefficient ($r_{xy}$). That is, a correlation is a covariance between $z$ scores. Because correlations are easier to interpret than covariances, it is common to report the means, *SD*s, and correlations. Using the *SD*s, readers can rescale correlations to covariances in the orignal units of measurement (i.e., $\sigma_{xy} = \sigma_x \times \sigma_y \times r_{xy}$). The `cov2cor()` function can standardize a covariance matrix, or you can use the `cor()` function directly.
...
An outcome $\mathrm{y}$ is a linear combination (weighted sum) of predictors

-   Like a recipe for the outcome: How much of $x_1$ and $x_2$ do you need to recreate $\mathrm{y}$?

```{=tex}
\begin{align*}
y_i &= \color{blue}{\beta_0 + \beta_1 x_{i1} + \ldots + \beta_p x_{ip}} &+& \color{darkred}{\varepsilon_i} & \\
    &= \color{blue}{\beta_0 + \sum_{j=1}^p \beta_j x_{ij}} &+& \color{darkred}{\varepsilon_i} &\sim \color{darkred}{\mathcal{N}(0, \sigma)}
\end{align*}
```
-   The \color{blue}{deterministic component} of $\mathrm{y}$ can be predicted or explained by $\mathbf{X}$
-   The \color{darkred}{stochastic component} of $\mathrm{y}$ includes all other unmodeled effects (omitted variables) and sources of error
...
\end{equation*}
```
-   Shorthand: $\mathrm{y} = \mathbf{X} \mathbf{\beta} + \mathbf{\varepsilon}$
...
-   **Only the intercept** ($\beta_0$) is interpreted as a mean:
    -   The **expected value** ($\hat{\mathrm{y}}$) when each predictor $x=0$
-   Each slope ($\beta_j$) is interpreted as the **change** in $\hat{\mathrm{y}}$ as $x_j$ increases
    -   i.e., How does $\mathrm{y}$ **covary** with $x_j$, given other $x$s?

Ordinary Least Squares (OLS) estimator of coefficients minimizes overall discrepancies between observations and predictions ($\mathrm{y} - \hat{\mathrm{y}}$):

$$\beta = (\mathbf{X}^{'} \mathbf{X})^{-1} \mathbf{X}^{'} \mathbf{y}$$

-   Minimizing residual variance = maximizing $R^2$
-   $R$ = Cor($\mathrm{y}, \hat{\mathrm{y}}$)
...
dat <- foreign::read.spss("demoData/MODMED.sav",
...
-   OLS regression minimizes discrepancies between observed and predicted **casewise observations** of $\mathrm{y}$
    -   $\mathrm{y} - \hat{\mathrm{y}}$
-   LS and ML estimators in SEM minimize discrepancies between observed and predicted **summary statistics**
    -   mean vector: $\bar{\mathrm{y}} - \widehat{\mu}$
        -   $\bar{\mathrm{y}}$ = observed sample means
...
$$\text{Var}(\mathrm{y}) = \frac{\sum_{i=1}^N (\mathrm{y}_i - \bar{\mathrm{y}})^2}{N-1} = \frac{\sum_{i=1}^N (\mathrm{y}_i - \bar{\mathrm{y}})(\mathrm{y}_i - \bar{\mathrm{y}})}{N-1}$$

Covariance ($\sigma_{xy}$) quantifies how $x$ and $\mathrm{y}$ are linearly related

-   *co*vary = vary together

$$\text{Cov}(x,y) = \frac{\sum_{i=1}^N (x_i - \bar{x})(\mathrm{y}_i - \bar{\mathrm{y}})}{N-1}$$
...
$$\mathbf\Sigma = (\mathbf{Y} - \bar{\mathbf{Y}})^{'} (\mathbf{Y} - \bar{\mathbf{Y}})$$
...
**Slopes** are change in $\mathrm{y}$ (or $x$) per unit $x$ (or $\mathrm{y}$)
...
With mean-centered predictors, $(\mathbf{X}^{'} \mathbf{X})^{-1}$ is their covariance matrix and $\mathbf{X}^{'} \mathrm{y}$ captures their covariances with the outcome
...
In multiple regression, the same formulas additionally account for covariances of $x$ and $\mathrm{y}$ with covariates
...
```{=tex}
\begin{align*}
\mathrm{y}_i &= \beta_0 (1) + \varepsilon_i \\
    &= \bar{\mathrm{y}} + (\mathrm{y}_i - \bar{\mathrm{y}})
\end{align*}
```
-   the intercept is the mean
-   residuals are mean-centered $\mathrm{y}$ (same variance)

OLS simplifies to the arithmetic mean formula: $\bar{\mathrm{y}} = \frac{\Sigma_y}{N}$

-   $(\mathbf{X}^{'} \mathbf{X})^{-1} = \frac{1}{N}$
-   $\mathbf{X}^{'} \mathrm{y} = \sum y$
...
-   A mean ($\bar{\mathrm{y}}$) is an expected value under a particular condition ($\beta_0$ when $X=0$) plus predictors' means times their slopes: $\bar{\mathrm{y}} = \beta_0 + \sum \beta_j \bar{x}_j$
