...
The lavaan script for fitting a factor model resembles the script for fitting a path model. We again use equations to specify the model, but we now use the “`=~`” operator to define the common factors. Whereas the “`~`” operator specifies regression paths that point from predictor(s) on the right-hand side to outcome(s) on the left-hand side, the “`=~`” operator specifies regression paths (called factor loadings) that point from the predictor (common factor) on the left-hand side to the outcomes (indicators) on the right-hand side. Script 14.1 fits the three-factor model from Figure \@ref(fig:fig14-3) to the covariance matrix from observed scores of the SAQ that was administered to 915 school pupils.
...
The residual errors are specified in the same way as they are with the path model, using the double tildes (~~) to represent two-headed arrows. These are the residual terms that are represented in the theta matrix (Θ). As there are no covariances between residual factors specified in our model, this matrix is diagonal.  That is, the diagonal elements of the symmetric Θ are the indicators’ residual variances, and all residual covariances are fixed to zero. We use labels “TH11” through “TH99” to represent the 1st-row/1st-column element through the 9th-row/9th-column element.  
