...
$$
\widehat{\beta}^*_{32} = \widehat{\beta}_{32} \times \frac{\widehat{\sigma}_2}{\widehat{\sigma}_3} = 0.21 \times \frac{\sqrt{194.32}}{\sqrt{130.19}} = .26 .
$$
...
The calculation of standardized regression coefficients can be done for the complete matrix of estimated regression coefficients that are represented in matrix $\widehat{B}$. The matrix expression for the standardized $\widehat{B}$ matrix, $\widehat{B}^*$, is the following: 

```{=tex}
\begin{equation}
\widehat{B}^* = diag(\widehat{\Sigma})^{-\frac{1}{2}} \widehat{B} \hspace{1mm} diag(\widehat{\Sigma})^\frac{1}{2}
(\#eq:6-02)
\end{equation}
```
...
0&0&0&0\\
0&0&0&0\\
\widehat{\beta}_{31}^* & \widehat{\beta}_{32}^* &0&0\\
0&0&\widehat{\beta}_{43}^* &0\\
\end{bmatrix} =
\begin{bmatrix}
0&0&0&0\\
0&0&0&0\\
...
```{=tex}
\begin{equation}
COR_{21} = \frac{COV_{21}}{\widehat{\sigma}_1 \widehat{\sigma}_2} 
(\#eq:6-03)
\end{equation}
```
...
$$
COR_{21} = \frac{COV_{21}}{\widehat{\sigma}_1 \widehat{\sigma}_2} = \frac{53.36}{\sqrt{91.58 \times 194.32}} = .40.
$$
...
The matrix expression for standardized variances and covariances in the $\widehat{Ψ}$ matrix is:

```{=tex}
\begin{equation}
\widehat{Ψ}^* = diag(\widehat{\Sigma}_{model})^{-\frac{1}{2}} \widehat{Ψ} \hspace{1mm} diag(\widehat{\Sigma}_{model})^\frac{1}{2}
(\#eq:6-04)
\end{equation}
```
...
\widehat{\Psi}^* = 
\begin{bmatrix}
\widehat{\psi}_{11}^* & \widehat{\psi}_{21}^*&0&0\\
\widehat{\psi}_{12}^* & \widehat{\psi}_{22}^*&0&0\\
0&0 &\widehat{\psi}_{33}^*&0\\
0&0&0 &\widehat{\psi}_{44}^*\\
\end{bmatrix} =
\begin{bmatrix}
1 &0.40 & 0 & 0\\
0.40 & 1 & 0 & 0\\
...
There are several ways to obtain standardized parameter estimates from a path model. Here, we will show how to manually calculate standardized parameter estimates from the estimated ($\widehat{B}$ $\widehat{\Psi}$) and model-implied ($\widehat{\Sigma}$) matrices. 

Script 6.1 shows the syntax that standardizes the $\widehat{B}$ and $\widehat{\Psi}$matrices ($\widehat{B}^*$ and $\widehat{\Psi}^*$) from the output of the Affrunti & Woodruff-Borden model that was saved in the object `AWmodelOut`.
...
We also need the model-implied covariance matrix, $\widehatΣ$ model, which we can calculate with the parameter estimates, using $\widehat{\Sigma}_{model} = (I-\widehat{B})^{-1} \widehat{\Psi}[(I-\widehat{B})^{-1}]^T$. We already extracted the parameter estimates for the $\widehat{B}$ and $\widehat{\Psi}$ matrices (into the objects `BETA` and `PSI`), so now we create an identity matrix `IDEN` with dimensions 4 × 4 and then calculate $\widehatΣ_{model}$ using:
...
The $\widehat{B}$ matrix is standardized by pre-multiplying $\widehat{B}$ with a diagonal matrix with the inverse of the $SD$s, and post-multiplying it with a diagonal matrix with the $SD$s: 

```{r}
BETAstar <- solve(SD) %*% BETA %*% SD
```

The matrix $\widehat{\Sigma}$ is standardized by pre- and post-multiplying $\widehat{\Sigma}$ with a diagonal matrix with the inverse of the standard deviations. 
...
To view the standardized $\widehat{B}^*$  and $\widehat{\Sigma}^*$ matrices, type: 
